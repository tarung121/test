...
Scenario Outline: Get Fees Details Validation
    ...
    * string productName = <ProductTypes>+''
     * def totalFees = licenseFees  + fillingFees
     And print totalFees	
   				   And request {"mainLicensePermitTypeId":'#(mainLicensePermitTypeId)',"newPermitTypeIds":[],"combinedCraftId":null,"masterFileId":null,"isApplicableForTempPermit":false,"isNotQualified":false,"isChainRestaurant":false,"addBarList":[],"countyId":3185,"isApplicantCurrentLicensed":false,"isExistingManufacturer":false,"selectedLicense":null}
	  When method post
	  Then status 200
        ...
        * def addLicenseDataOnMismatchedFunc =
			...
			function(actfilingFees,expfilingFees,actInitialLicenseFee, expInitialLicenseFee,acttermActRes,exptermActRes,actbondFee,expBondValue) {
			        ...
			        if(actfilingFees != expfilingFees || actInitialLicenseFee != expInitialLicenseFee || acttermActRes != exptermActRes || actbondFee != expBondValue ){
			            ...
			            return db.appendStrToFile('UnitCategoryFees.csv' , ('\n'+lDesc+','+Units+','+productName+','+CitysName+','+actfilingFees+','+expfilingFees+','+actInitialLicenseFee+','+expInitialLicenseFee+','+acttermActRes+','+exptermActRes+','+actbondFee+','+expBondValue));
			        }
	        }
			"""
  ...
  Examples:
   | read('/LicenseInputs/UnitCategoryFees.csv') |