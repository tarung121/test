	...
	Given path '/internalapi/api/licensing/selectapptype/savenewlicenseapp'
      ...
      * def renewalFees = <RenewalFilingFees>+''
       ...
       * def totalFees = licenseFees  + fillingFees
         And print totalFees	
   	     And request {"mainLicensePermitTypeId":'#(mainLicensePermitTypeId)',"noOfMonths":'#(termInMonths)',"startDate":'#(dt)',"endDate":'#(dtEnd)',"newPermitTypeIds":[],"combinedCraftId":null,"masterFileId":null,"isApplicableForTempPermit":false,"isNotQualified":false,"isChainRestaurant":false,"addBarList":[],"countyId":5352,"isApplicantCurrentLicensed":false,"isExistingManufacturer":false,"selectedLicense":null}
	     When method post
	     Then status 200
...
# ********* SAVE Application Details To Save *********************
        ...
        * def addLicenseDataOnMismatchedFunc =
			...
			function(actfilingFees,expfilingFees,actInitialLicenseFee, expInitialLicenseFee,acttermActRes,exptermActRes,actbondFee,expBondValue) {
			        ...
			        if(actfilingFees != expfilingFees || actInitialLicenseFee != expInitialLicenseFee || acttermActRes != exptermActRes || actbondFee != expBondValue ){
			            ...
			            return db.appendStrToFile('LiquorRetailCityFees.csv' , ('\n'+lDesc+','+productName+','+CitysName+','+actfilingFees+','+expfilingFees+','+actInitialLicenseFee+','+expInitialLicenseFee+','+acttermActRes+','+exptermActRes+','+actbondFee+','+expBondValue));
			        }
	        }
			"""
         * def addLicenseDataOnMismatched = addLicenseDataOnMismatchedFunc(filingFeesAct,fillingFees,licensingFees,licenseFees,termActRes,termInMonths,bondFee,BondValue)
 ...
 Examples:
   | read('/LicenseInputs/LiquorRetailCityFees.csv') |