Feature: Liquor Retail License Submission
Background:
			* url BaseURL
...
Scenario Outline: Wholesale License intake
	    ...
	    * call read('LicensesCommonMethods.feature@IntakeRetailonLiqourPremise') {}
	    ...
	    * call read('LicensesCommonMethods.feature@Save500FootHearing') {}
	    * call read('LicensesCommonMethods.feature@Submit500FootHearing') {}
       ...
       * call read('RenewalCommonMethods.feature@Fee_Check') {}
        * match RenewalFilingFees == <RenewalFilingFees>
            
        * def appendAppID = db.appendStrToFile('licenseAPPIds.csv' , ('\n'+mainappId+','+termDesc+','+LicenseDescription+','+licenseId+','+ApplicationId))
 	
	
	 Examples:
    | read('/LicenseInputs/LiqourRetailOnPremises.csv') |
