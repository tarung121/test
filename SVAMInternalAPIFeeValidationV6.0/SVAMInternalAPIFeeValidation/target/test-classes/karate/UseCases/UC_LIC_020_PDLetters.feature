...
@TC0001_NYS_SLA_LIC_PD_Letter_On	  
Scenario Outline: TC0001_NYS_SLA_LIC_PD_Letter_On
	...
	# ********* Login Functionality *********************
  	* callonce read('LoginDetails.feature') { strToken:'#(strToken)'}
	    ...
	    * def appendAppID = db.appendStrToFile('20DayLetter.csv' , ('\n'+appId+','+termDesc+','+LicenseDescription+','+Address1+','+Address2+','+legalName+','+CityName+','+zipCode+','+countryName))
...
Examples:
    | read('/CSVFiles/TC0001_NYS_SLA_LIC_PD_Letter_On.csv') |    
	  

@TC0002_NYC_SLA_LIC_PD_Letter_Of	  
Scenario Outline: TC0002_NYC_SLA_LIC_PD_Letter_Of
	...
	# ********* Login Functionality *********************
  	* callonce read('LoginDetails.feature') { strToken:'#(strToken)'}
	   ...
	   ## ******************** UC_LIC_020 b: Validate TC0002_NYC_SLA_LIC_PD_Letter_Of Scenario Start ***************************##
	    ...
	    #* call read('LicensesCommonMethods.feature@ValidateCountyNameList') {}	
	    * call read('LicensesCommonMethods.feature@IntakeLicensewithoutAssociatedLic') {}
	    * def CountyName = "New York"
	    * call read('LicensesCommonMethods.feature@FillAndSaveApplicantInformationPage1') {county:'#(CountyName)'}
	    * call read('LicensesCommonMethods.feature@FillAndSavePrincipalPage') {}
	    * call read('LicensesCommonMethods.feature@FillAndSaveRepresentativePage') {}
	    * call read('LicensesCommonMethods.feature@WaveFeesValidation') {expStatus:'Intake Complete'}
	    * call read('LicensesCommonMethods.feature@SubmitLicenseAndValidateLicStatus2') {expStatus:'Awaiting Review'}
	   ...
	   * def docName = 'Email'
	    ...
	    * call read('LicensesCommonMethods.feature@ValidateCommunicationPageAndEMailForOFFPromises') {NotificationMethodName:'SendPDLetterNotification',typeOfNotification:'Email',emailBodyData:'#(emailBodyData)'}
